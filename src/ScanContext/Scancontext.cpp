...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
    ...
    if (tree_making_period_conter % TREE_MAKING_PERIOD_ == 0)  // to save computation cost
    {
        ...
        // tree_ptr_->index->buildIndex(); // inernally called in the constructor of InvKeyTree (for
        // detail, refer the nanoflann and KDtreeVectorOfVectorsAdaptor)
         ...
         candidate_iter_idx++)
    {
        ...
        std::pair<double, int> sc_dist_result = distanceBtnScanContext(curr_desc,
                                                                       polarcontext_candidate);
    ...
    {
        std::cout.precision(3);
        cout << "[Not loop] Nearest distance: " << min_dist << " btn " << polarcontexts_.size() - 1
             << " and " << nn_idx << "." << endl;
